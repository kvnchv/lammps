...
void SetCAC::command(int narg, char **arg)
{
  if (domain->box_exist == 0)
    error->all(FLERR,"SetCAC command before simulation box is defined");
  if (atom->natoms == 0)
    error->all(FLERR,"SetCAC command with no atoms existing");
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
      ...
      if (index_custom < 0 || flag != 0)
        error->all(FLERR,"SetCAC command integer vector does not exist");
      set(INAME);
      iarg += 2;
      ...
      if (index_custom < 0 || flag != 1)
        error->all(FLERR,"SetCAC command floating point vector does not exist");
      set(DNAME);
      iarg += 2;
...
void SetCAC::selection(int n)
{
  ...
  if (style == ATOM_SELECT) {
    ...
    if (iregion == -1) error->all(FLERR,"SetCAC region ID does not exist");
...
void SetCAC::setrandom(int keyword)
{
  int i;
...
void SetCAC::topology(int keyword)
{
...
void SetCAC::varparse(char *name, int m)
{
  varflag = 1;
