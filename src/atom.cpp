  ...
  int shiftflag, double *shift)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (values[2] == NULL)
      error->one(FLERR, "Incorrect atom format in data file");
    npoly = utils::inumeric(FLERR,values[2],false,lmp);
    if (npoly < 1)
    ...
    for (int dimcheck=0; dimcheck<3; dimcheck++) {
      ...
      if (values[3+dimcheck] == NULL)
        error->one(FLERR, "Incorrect atom format in data file");
      escale=utils::inumeric(FLERR,values[3+dimcheck],false,lmp);
      if (escale<1)
    ...
    if (imageflag)
      imagedata = ((imageint)(utils::inumeric(FLERR,values[iptr],false,lmp) + IMGMAX) & IMGMASK) |
        (((imageint)(utils::inumeric(FLERR,values[iptr+1],false,lmp) + IMGMAX) & IMGMASK) << IMGBITS) |
        (((imageint)(utils::inumeric(FLERR,values[iptr+2],false,lmp) + IMGMAX) & IMGMASK) << IMG2BITS);
    else
    ...
    for (int mm = 0; mm < nodecount*npoly; mm++) {
      xdata[0] += utils::numeric(FLERR,values[decline + xptr + mm*words_per_node],false,lmp);
      xdata[1] += utils::numeric(FLERR,values[decline + xptr + 1 + mm*words_per_node],false,lmp);
      xdata[2] += utils::numeric(FLERR,values[decline + xptr + 2 + mm*words_per_node],false,lmp);
    }
